/**
 * From LANDED to TAKINGOFF, it uses the frame "odom -> base_link" to navigate because the map has
 * not been generated by RTAB-Map. But why can't it generate map before the whole Navigator is
 * launched? The main reason is that when in `openvins` mode, we need `groundtruth` odometry to fly
 * the controller. As soon as the `groundtruth` odometry is published, the map will be generated
 * according to the `groundtruth` odometry. Once it switches to `openvins` odometry, the generated
 * map will have odometry inconsistency. I.e., the odometry jumps will give the overlapping/aliasing
 * map. Thus, I think the best approach is to use `groundtruth` odometry for takeoff. During
 * takeoff, it switches to `openvins` mode, then the map is generated afterwards solely based on the
 * `openvins` odometry. This way, the map is consistent with the `openvins` odometry and the
 * navigation can proceed without worrying about the odometry inconsistency issue.
 */
